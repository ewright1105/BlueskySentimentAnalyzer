...
def add_query(data):
    try:
        ...
        try:
            num_intervals = int(data["NumIntervals"])
            interval_length = int(data["IntervalLength"])
            posts_to_analyze = int(data["PostsToAnalyze"])
            interval_unit = "minutes"
            if interval_unit not in ["minutes", "hours", "days"]:
                raise ValueError("Invalid IntervalUnit. Must be 'minutes', 'hours', or 'days'.")
            if num_intervals <= 0 or interval_length <= 0:
                 raise ValueError("NumIntervals and IntervalLength must be positive integers.")
        except (ValueError, TypeError) as e:
             raise ValueError(f"Invalid numeric value or IntervalUnit: {e}")
        # schedule_unit = interval_unit if interval_unit.endswith('s') else f"{interval_unit}s"
        schedule_unit = "minutes";
        if interval_length == 1:
             schedule_unit = schedule_unit.rstrip('s') # Singular for value 1
        schedule_expression = f"rate({interval_length} {schedule_unit})"
